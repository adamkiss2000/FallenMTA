...
for k, v in pairs(skinData) do
    ...
    if not v[3] and not v[4] then
        if not skinByGender[gender] then
            skinByGender[gender] = {}
        end
        table.insert(skinByGender[gender], k)
    end
end

function generateRandomSkin(gender)
    if skinByGender[gender] then
        return skinByGender[gender][math.random(1, #skinByGender[gender])]
    end
end
...
function checkSkin(source, dName, oValue)
   ...
   --outputChatBox("asd1")
    ...
    if a3 ~= typ and a3 ~= "*" then
        ...
        source:setData(dataName, generateRandomSkin(typ))
        return
    end
...
addEventHandler("onClientResourceStart", resourceRoot,
    function()
        if localPlayer:getData("loggedIn") then
            ...
            local typ = a["neme"]
            checkSkin(localPlayer, "char >> skin", generateRandomSkin(typ))
        end
    end
)
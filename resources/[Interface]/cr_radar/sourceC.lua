...
function renderWidget.minimap(x, y, w, h)
	...
	if minimapPosX ~= x or minimapPosY ~= y then
		...
		dxDrawRectangle(minimapPosX, minimapPosY, minimapWidth, 22, tocolor(0,0,0, 255 * 0.35))
		...
		local font_bold = exports['cr_fonts']:getFont("Hand", 17)
		local text_width = dxGetTextWidth( zoneName, 1, font_bold )
		if w_w > text_width + 32 then 
			dxDrawText(zoneName, minimapPosX+30, minimapPosY, minimapPosX + minimapWidth - 20, minimapPosY + 27, tocolor(242, 242, 242, 255), 1, font_bold, "left", "center",false,false,false,true)
		end
...
function renderTheBigmap()
	...
	if playerDimension == 0 then
		...
		--local scale = 2
		--dxDrawImage(bigmapPosX + bigmapWidth - 176 * scale, bigmapPosY + bigmapHeight - 23 * scale, 176 * scale, 23 * scale, "assets/images/design/footer.png")
...
addEventHandler("onClientKey", getRootElement(),
	function (key, pressDown)
		...
		elseif key == "mouse_wheel_down" then
			if pressDown then
				if exports['cr_core']:isInSlot(bigmapPosX, bigmapPosY, bigmapWidth, bigmapHeight) and bigmapIsVisible and bigmapZoom - 0.1 >= 0.5 then
					bigmapZoom = bigmapZoom - 0.1
				end

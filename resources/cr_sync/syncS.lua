...
function updateTime()
    ...
    if realTime.month >= 0 and realTime.month <= 2 or realTime.month >= 9 then
...
function fetchWeather()
    ...
    fetchRemote("http://api.weatherapi.com/v1/current.json?key=2ce9bf01a4d64347bb1193326202705&q=Budapest", function(data)
        ...
        if gtaNames[weather] then
            ...
            if gtaNames[weather] == 15 then 
                setRainLevel(math.random(1, 10) / 10);
            end
        else
            setWeather(1);
        end

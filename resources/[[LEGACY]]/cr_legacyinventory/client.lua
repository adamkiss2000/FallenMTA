...
function openInventory(e, spec, def)
    ...
    CreateNewBar("stack", {x+drawnSize["bg"][1]-60, y + 12.5, 50, 20}, {4, "", true, tocolor(255,255,255,255), {"Poppins-Regular", 10}, 1, "center", "center"}, 1)
...
function drawnInventory()
    ...
    x, y = getNode("Inventory", "x"), getNode("Inventory", "y")
    UpdatePos("stack", {x+drawnSize["bg"][1]-60, y + 12.5, 50, 20})
    ...
    dxDrawRectangle(_x, _y, w, h+5, tocolor(40,40,40,alpha * 0.9)) -- háttér
    dxDrawRectangle(_x, _y, w, 40, tocolor(28, 28, 28, alpha * 0.9))
	dxDrawRectangle(_x+w-60, _y + 10, 50, 20, tocolor(40,40,40, alpha * 0.9))
    ...
    elseif invType == specTypes["object"] then
        ...
        for k,v in pairs(iconPositions) do
            ...
            if invType == typeID or isInSlot(_x + ax+175, _y + ay, w, h) then
                dxDrawImage(_x + ax+175, _y + ay, w, h, "assets/images/"..name..".png", 0,0,0, tocolor(255,59,59,alpha))
            else
                dxDrawImage(_x + ax+175, _y + ay, w, h, "assets/images/"..name..".png", 0,0,0, tocolor(242,242,242,alpha * 0.6))
            end
        end
    end
    ...
    if invType == 5 then
        ...
        if isInSlot(_x + 270, _y + 240, 155, 30) then
            dxDrawRectangle(_x + 270, _y + 240, 155, 30, tocolor(255,59,59,alpha))
            cActive = 11 -- 11 / Button
            local x,y,w,h = _x + 270, _y + 240, _x + 270 + 155, _y + 240 + 30
            dxDrawText("Elkészít", x, y+5, w, h, tocolor(242, 242, 242,alpha), 1, font3, "center", "center")
        else
            dxDrawRectangle(_x + 270, _y + 240, 155, 30, tocolor(255,59,59, alpha * 0.7))
            local x,y,w,h = _x + 270, _y + 240, _x + 270 + 155, _y + 240 + 30
            dxDrawText("Elkészít", x, y+5, w, h, tocolor(242,242,242,alpha * 0.6), 1, font3, "center", "center")
        end
        ...
        dxDrawRectangle(_x+10, _y + h -5, _w-20, 5, tocolor(242,242,242,alpha * 0.6))
        dxDrawRectangle(_x+10, _y + h -5, (_w-20) * multipler, 5, tocolor(255,59,59,alpha))
        if isInSlot(_x+10, _y + h -5, _w-20, 5) then
            local specData = {["alpha"] = alpha}
                if data["size"] then
                    specData["size"] = 2
                end
                text = math.floor(multipler * 100) .. "%"
            local cx, cy = exports['cr_core']:getCursorPosition()
            exports['cr_dx']:drawTooltip(2, text, {cx, cy,specData})
        end
        ...
        for i = 1, maxLines * maxColumn do
            ...
            if isInSlot(startX, startY, w, h) then
                dxDrawRectangle(startX, startY, w, h, tocolor(255,59,59,alpha))
                isIn = true
                _lastItem = lastItem
                lastItem = i
            else
            ...
            if b == "left" and s == "down" then
                ...
                if not moveState and not craftMoveState then
                    if invElement.type == "player" then
                        for k,v in pairs(iconPositions) do
                            local ax, ay, name, typeID, w, h = unpack(v)
                            if isInSlot(_x + ax+175, _y + ay, w, h) then
                                if typeID ~= 5 then

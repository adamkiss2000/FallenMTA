...
addEventHandler("onResourceStart", resourceRoot, function()
	...
	for i, k in pairs(mapsdatas) do
		...
		elseif k[1] == "create" then
			...
			local frozen = k.frozen;

			local object = createObject(model, x, y, z, rotx, roty, rotz);
			if object then
				setElementAlpha(object, tonumber(alpha) or 255)
				setObjectScale(object, tonumber(scale) or 1)
                if (doublesided and true or false) then
                    setElementDoubleSided(object, doublesided and true or false);    
                end
				if (frozen and true or false) then
		    		setElementFrozen(object, frozen and true or false);
				end
                ...
                if (breakable and true or false) then
				    --setElementBreakable(object, breakable and true or false);
	...
	--("Map loaded in "..((endTick-startTick)).."ms. ("..infos["remove"].." remove & "..infos["create"].." create)");
end);

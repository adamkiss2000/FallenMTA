local exportPath = ":lm_maps/";
local buildPath = ":lm_maploader/";
...
addCommandHandler("compilemap", function(thePlayer)
    if not exports['lm_core']:getPlayerDeveloper(thePlayer) then return end
	compileMap();
end);

function compileMap()
	local resource = getResourceFromName("lm_maploader");
	if resource then
		local state = exports.lm_builder:unbuildResource(resource);
	end
    ...
    exports.lm_builder:processResource(resource);
end

...
function loadModells() 
    ...
    if not loadingStarted then
        ...
        for k, v in ipairs(cache) do
            local model, fileSrc, typ, isGlass, isVirtualVehicle = unpack(v)
        ...
        startTimer = setTimer(
            function()
                    ...
                    if tonumber(model) then
                        ...
                        elseif typ == "dff" then
                            ...
                            if dff then
                                ...
                                outputDebugString("[VEHICLES] DFF load failed: "..tostring(fileSrc), 2)
                            end
                ...
                if #loadCache == 0 then
                    ...
                    outputDebugString(
                        "Loading succesfully finished, #"..allStart.." vehicles loaded in #"..seconds.." seconds!",
                        0, 20, 87, 255
                    )
...
function loadModel(id)
    if not idCache[tostring(id)] then return end

    for _, v in pairs(idCache[tostring(id)]) do
        local model, fileSrc, typ, isGlass, isVirtualVehicle = unpack(v)

...
function debugRender()
    ...
    for k,v in ipairs(cache) do 
        ...
        if count ~= 1 then 
            dxDrawText("["..side.."] [x"..count.."] "..message.. " -> Line: "..(line or "n/a"), debugX+10, debugY+200-(k*20), 500, 200, tocolor(colors[level][1],colors[level][2],colors[level][3]), 1, font, "left");
        else 
            dxDrawText("["..side.."] "..message.. " -> Line: "..(line or "n/a"), debugX+10, debugY+200-(k*20), 500, 200, tocolor(colors[level][1],colors[level][2],colors[level][3]), 1, font, "left");
        end
    end
end

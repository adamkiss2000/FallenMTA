...
function govAnnouncement(cmd, ...)
    if localPlayer:getData("loggedIn") then 
        ...
        --if dutyFaction then 
        local canGov = false
        for k, v in pairs(govGroups) do
            if exports.cr_dashboard:isPlayerFactionLeader(localPlayer, k) then
                canGov = true
                if canGov then 
                    local factionName = exports.cr_dashboard:getFactionName(k)
    ...
    if hasPlayerPermission(localPlayer, "changelock", "faction.changelock") then 
        ...
        if isElement(vehicle) then 
            ...
            --if dutyFaction then 
                ...
                if vehFaction and exports.cr_dashboard:isPlayerFactionLeader(localPlayer, vehFaction) then 

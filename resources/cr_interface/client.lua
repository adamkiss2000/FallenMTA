		...
		if moveingWidget then
            ...
            if widgets.isInBox(sx/2 - 70, 20, 60, 60) then
                if cfg[k]["turnable"] then
                    ...
                    localPlayer:setData(k..".enabled", false)
                    if k ~= "box" then
                        if cfg[k]["name"] and cfg[k]["name"] ~= "" then
...
function jsonGET2(file)
    ...
    if not fileExists(file) then
        ...
        local tables = {}
        for k,v in pairs(defPositions) do
            if not v["showing"] and v["name"] and v["name"] ~= "" then
    ...
    if fileHandle then
        ...
        local temp = fromJSON(allBuffer)
        jsonDATA = {}

        for i, widgetID in ipairs(temp) do
            local def = defPositions[widgetID]
            if def and def.name and def.name ~= "" then
                table.insert(jsonDATA, widgetID)
            end
        end
        fileClose(fileHandle)
    end

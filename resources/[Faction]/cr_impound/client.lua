...
local impoundPed = Ped(21, 1395.4320068359, 452.36059570312, 19.988025665283)
    ...
    function()
        if impoundDatas then 
            if exports["cr_core"]:takeMoney(localPlayer, tonumber(impoundDatas[8]), nil, false) then 
                ...
                -- Frissítés: vegye ki a listából a járművet
                table.remove(cache, hoverButton)
                ...
                if count <= _maxLines then
                    minLines = 1
                    maxLines = count
                elseif maxLines > count then
                    maxLines = count
                    minLines = math.max(1, maxLines - (_maxLines - 1))
                end

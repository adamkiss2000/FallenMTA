...
addEventHandler("impound.takeVehicle", root,
    function(thePlayer, data)
        ...
        local vehicleId = tonumber(data[4])
        local ownerId = tonumber(data[2])

        -- Törlés adatbázisból
        dbExec(sql, "DELETE FROM impound WHERE id = ?", id)

        -- Cache frissítés
        if cache[ownerId] then
            for i, v in ipairs(cache[ownerId]) do
                if v[1] == id then
                    table.remove(cache[ownerId], i)
                    break
                end
            end

            if #cache[ownerId] == 0 then
                cache[ownerId] = nil
            end
        end
        ...
        outputChatBox(exports["cr_core"]:getServerSyntax("Impound", "success") .. "Sikeresen kiváltottad a járművet!", thePlayer, 255, 255, 255, true)
    end
)
...
addCommandHandler("lefoglal",
    function(thePlayer, cmd)
        ...
        if vehicle then
            local faction = thePlayer:getData("char >> faction") or "Unknown"
            impoundVehicle(thePlayer, vehicle, faction)
        else
            outputChatBox("Nem vagy járműben!", thePlayer, 255, 0, 0)
        end
    end
)
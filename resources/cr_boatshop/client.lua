...
cDetails = {
	...
	worldTime = {0, 0},
	position = Vector3(2158.1137695312, -104.41160583496, 1.55),
	marker = false,
	greenBox = false,
	controllerPed = false,
	vehiclePreviewPosition = Vector3(2089.1716308594, -32.042663574219, 0),
	mode = "show",
	isShowing = false,
	selectedBrand = 1,
...
function renderBoatshop()
	...
	dxDrawText('Hajókereskedés', 115, 100/2, 115, 100/2 + 4, tocolor(242, 242, 242, alpha), 1, font, "left", "center")
	...
	local tHeight = dxGetFontHeight(1, font2)
	dxDrawRectangle(sx - 20 - tWidth, startY, tWidth, tHeight, tocolor(51, 51, 51, alpha * 0.8))
	exports['cr_dx']:dxDrawImageWithText(":cr_boatshop/files/imgs/cardIcon.png", moneyText, sx - 20 - tWidth + tWidth/2, startY, sx - 20 - tWidth + tWidth, startY + tHeight + 4, tocolor(r,g,b, alpha), tocolor(242, 242, 242, alpha), 20, 15, 1, font2, 5, -2)
	...
	if exports['cr_core']:isInSlot(x + w - 150, y - 5 - 30, 150, 30) then 
		...
		exports['cr_dx']:dxDrawImageWithText(":cr_boatshop/files/imgs/testdriveIcon.png", 'Tesztvezetés', x + w - 150 + 150/2, y - 5 - 30, x + w - 150 + 150/2, y - 5 - 30 + 30 + 4, tocolor(51, 51, 51, alpha), tocolor(51, 51, 51, alpha), 20, 20, 1, font5, 5, -2)
	else 
		dxDrawRectangle(x + w - 150, y - 5 - 30, 150, 30, tocolor(51, 51, 51, alpha * 0.6))

		exports['cr_dx']:dxDrawImageWithText(":cr_boatshop/files/imgs/testdriveIcon.png", 'Tesztvezetés', x + w - 150 + 150/2, y - 5 - 30, x + w - 150 + 150/2, y - 5 - 30 + 30 + 4, tocolor(242, 242, 242, alpha * 0.6), tocolor(242, 242, 242, alpha * 0.6), 20, 20, 1, font5, 5, -2)
	end 
	...
	elseif(b == "mouse1") then
		if(state) then
			...
			elseif testDriveHover then 
				...
				triggerEvent("boatshop.testDrive", localPlayer)
...
addEvent('boatshop.testDrive.activate', true)
addEventHandler('boatshop.testDrive.activate', localPlayer, 
	function()
		triggerLatentServerEvent("boatshop>>testDrive", 5000, false, localPlayer, localPlayer, carList[cDetails.selectedBrand].vehicles[cDetails.selectedVehicle])
			...
			function()
				...
				if seconds <= 0 then
					...
					if minutes < 0 then
						...
						triggerLatentServerEvent("boatshop.exitTestDrive", 5000, false, localPlayer, localPlayer)
					end
				end
			end, 1000, 0
		)
					...
					if tonumber(exports['cr_vehicle']:getVehicleCounts()[data.model] or 0) + 1 <= data.limit or data.limit <= 0 then
						if getPlayerVehicles() + 1 <= tonumber(localPlayer:getData("char >> vehicleLimit") or 0) then 
							if tuningPrice[1] == 0 or exports['cr_core']:takeMoney(localPlayer, tuningPrice[1]) then 
								...
								exports['cr_infobox']:addBox("success", "Sikeresen megvetted a hajót $ "..data.price.." ért")
							else 
				...
				elseif type == 3 then 
					if getPlayerVehicles() + 1 <= tonumber(localPlayer:getData("char >> vehicleLimit") or 0) then 
						if tuningPrice[3] == 0 or localPlayer:getData("char >> premiumPoints") >= tuningPrice[3] then 
							...
							toggleBoatshop()
							exports['cr_vehicle']:makeVehicle(localPlayer, data.model, 0, localPlayer:getData("acc >> id"), toJSON(generatePosition()), toJSON({getVehicleColor(cDetails.vehicle, true)}))
							exports['cr_infobox']:addBox("success", "Sikeresen megvetted a hajót "..data.pp.." pp-ért")
						else
...
function toggleBoatshop()
	if(not isTimer(controlTimer)) then
		...
		if(cDetails.isShowing) then
			...
			cDetails.controllerPed:setPedControlState("vehicle_left", true)
			...
			--addEventHandler("onClientRender", root, renderBoatshop)
			createRender("renderBoatshop", renderBoatshop)
			...
			controlTimer = setTimer(function() 
				showCursor(false)
				--removeEventHandler("onClientRender", root, renderBoatshop)
				destroyRender("renderBoatshop")
...
addEvent("toggleBoatshop", true)
addEventHandler("toggleBoatshop", root, toggleBoatshop)
...
function init()
	...
	cDetails.marker:setData("marker >> customIconPath", ":cr_boatshop/files/icon.png")
end
...
local positions = {
	...
	{2080.0, -140.0, 3.4, 0, 0, 123, 0, 0},
	{2070.0, -125.0, 3.4, 0, 0, 130, 0, 0},
	{2060.0, -110.0, 3.4, 0, 0, 130, 0, 0},
}

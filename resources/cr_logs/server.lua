            ...
            function(queryHandle)
                ...
                if result and type(result) == "table" and #result > 0 then
                    for i, row in ipairs(result) do
                        local tableData = fromJSON(tostring(row["table"]))
                        if tableData then
                            tables = tableData
                        end
                    end
                else
                    outputDebugString("[logs] Nincs adat a tablesave táblában vagy dbPoll hibás!", 1)
                end
            end, connection, "SELECT `table` FROM `tablesave` WHERE `ID`=1"
        )
...
addEventHandler("onResourceStop", resourceRoot,
    function()
        dbExec(connection, "UPDATE `tablesave` SET `table`=? WHERE `ID`=1", toJSON(tables))
    end
)

...
function startCharReg()
    --requestTextBars
    cSelected = 1
    setCameraMatrix(1055.4074707031, 1024.9527587891, 15.854399681091, 1054.83203125, 1024.2176513672, 15.495995521545)
    stopSituations()
    details = {}
    stopLoginPanel()
    stopLoginSound()
    ...
    local skin = pSkins[cSelected][id]
    skinPed = createPed(skin, 1047.3057861328, 1016.040222168, 11, 321)
...
function stopCharacterRegistration()
    ...
    details["skin"] = pSkins[cSelected][id] --skinPed.model
    skinPed:destroy()
            ...
            if iState == "Char-Reg>Panel" then
                if b == "left" and s == "down" then
                    ...
                    elseif cHover == "woman" then
                        cSelected = 2
                        local newSkin = pSkins[cSelected][id]
                        if not newSkin then
                            id = 1
                            newSkin = pSkins[cSelected][id]
                        end
...
addEventHandler("loadCharacter", localPlayer,
    function(data)
        ...
        local neme, age, born, height, weight, fightStyle, walkStyle, description = a["neme"], a["age"], a["born"], a["height"], a["weight"], a["fightStyle"], a["walkStyle"], a["description"]
        if not description then
            local a = "Ő egy XX cm magas, XY kg súlyú, XZ éves ember!"
            a = a:gsub("XX", height)
            a = a:gsub("XY", weight)
            a = a:gsub("XZ", age)
            description = a

...
addEventHandler("player.banCheck", root, 
    function(player)
        local serial = getPlayerSerial(player)
        local ip = getPlayerIP(player)
        local identityKey = serial .. "-" .. ip

        if isIdentityHaveBan[identityKey] then
            triggerClientEvent(player, "banResult", player, true)
        else
            triggerClientEvent(player, "banResult", player, false)
        end
    end
)
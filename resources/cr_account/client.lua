...
function onLoginStart()
    local fallbackData = {
        ["Username"] = encodeString("base64", "Felhasználónév"),
        ["Password"] = encodeString("base64", "Jelszó"),
        ["Clicked"] = false,
        ["haveRPTest"] = false,
        ["haveFirstTour"] = false,
        ["soundVolume"] = 1,
    ...
    local data = exports['cr_json']:jsonGET("login/data.json", true, fallbackData)
    if not data then
        outputDebugString("[cr_account] jsonGET hibás: nil -> Line: 18", 1)
        data = fallbackData
    end
    ...
    if saveJSON["Username"] ~= "" and saveJSON["Username"] ~= " " then
        saveJSON["Username"] = decodeString("base64", saveJSON["Username"] or "")
    end

    if saveJSON["Password"] ~= "" and saveJSON["Password"] ~= " " then
        saveJSON["Password"] = decodeString("base64", saveJSON["Password"] or "")
    end
...
function banResult(boolean)
    ...
    if boolean then
        ...
        if not localPlayer:getData("loggedIn") then
            ...
            setTimer(
                function()
                    if isElement(localPlayer) and not getElementData(localPlayer, "char >> afk") then
                        local oPlayTime = getElementData(localPlayer, "char >> playedtime")
                        if type(oPlayTime) == "number" then
...
function onLoginStop()
    if saveJSON["Clicked"] then
        if saveJSON["Username"] ~= "" and saveJSON["Username"] ~= " " then
            saveJSON["Username"] = encodeString("base64", saveJSON["Username"])
        else
            saveJSON["Username"] = encodeString("base64", "Felhasználónév")
        end

        if saveJSON["Password"] ~= "" and saveJSON["Password"] ~= " " then
            saveJSON["Password"] = encodeString("base64", saveJSON["Password"])
        else
            saveJSON["Password"] = encodeString("base64", "Jelszó")
        end
    else
        saveJSON["Username"] = encodeString("base64", "Felhasználónév")
        saveJSON["Password"] = encodeString("base64", "Jelszó")
